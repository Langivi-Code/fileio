...
static php_socket_t php_uv_zval_to_valid_poll_fd(zval *ptr) {
    ...
    if (Z_TYPE_P(ptr) == IS_RESOURCE) {
        if ((stream = (php_stream *) zend_fetch_resource_ex(ptr, NULL, php_file_le_stream()))) {
            ...
            if (stream->wrapper && !strcmp((char *) stream->wrapper->wops->label, "PHP") && (!stream->orig_path ||
                                                                                             (strncmp(stream->orig_path,
                                                                                                      "php://std",
                                                                                                      sizeof("php://std") -
                                                                                                      1) &&
                                                                                              strncmp(stream->orig_path,
            ...
            fd = -1;
               ...
               (socket = (php_socket *) ((char *) (Z_OBJ_P(ptr)) - XtOffsetOf(php_socket, std)))) {
        fd = socket->bsd_socket;
#endif
    }
...
static void php_uv_cb_init(php_uv_cb_t **result, php_uv_t *uv, zend_fcall_info *fci, zend_fcall_info_cache *fcc,
                           enum php_uv_callback_type type) {
    php_uv_cb_t *cb;
...
PHP_UV_FS_ASYNC(loop, command, __VA_ARGS__);
    ...
    switch (fs_type) {
        ...
        case UV_FS_SENDFILE: {
            ...
            PHP_UV_FS_PARSE_PARAMETERS(4, Z_PARAM_RESOURCE(z_instream) Z_PARAM_RESOURCE(z_outstream) Z_PARAM_LONG(
                    offset) Z_PARAM_LONG(length));
            PHP_UV_FS_SETUP()
            /* TODO */
...
static inline zend_bool php_uv_closeable_type(php_uv_t *uv) {
    ...
    return ce == uv_pipe_ce || ce == uv_tty_ce || ce == uv_tcp_ce || ce == uv_udp_ce || ce == uv_prepare_ce ||
           ce == uv_check_ce || ce == uv_idle_ce || ce == uv_async_ce || ce == uv_timer_ce || ce == uv_process_ce ||
           ce == uv_fs_event_ce || ce == uv_poll_ce || ce == uv_fs_poll_ce || ce == uv_signal_ce;
}
...
static int
php_uv_do_callback2(zval *retval_ptr, php_uv_t *uv, zval *params, int param_count, enum php_uv_callback_type type
                    TSRMLS_DC) {
    int error = 0;
...
static void php_uv_udp_send_cb(uv_udp_send_t *req, int status) {
    ...
    if (!uv_is_closing(
            &uv->uv.handle)) { /* send_cb is invoked *before* the handle is marked as inactive - uv_close() will thus *not* increment the refcount and we must then not delete the refcount here */
...
static void
php_uv_udp_recv_cb(uv_udp_t *handle, ssize_t nread, const uv_buf_t *buf, const struct sockaddr *addr, unsigned flags) {
...
static inline zend_bool php_uv_is_handle_referenced(php_uv_t *uv) {
    ...
    return (ce == uv_signal_ce || ce == uv_timer_ce || ce == uv_idle_ce || ce == uv_udp_ce || ce == uv_tcp_ce ||
            ce == uv_tty_ce || ce == uv_pipe_ce || ce == uv_prepare_ce || ce == uv_check_ce || ce == uv_poll_ce ||
            ce == uv_fs_poll_ce) && uv_is_active(&uv->uv.handle);
}
...
static void php_uv_socket_bind(enum php_uv_socket_type ip_type, INTERNAL_FUNCTION_PARAMETERS) {
    ...
    if (ip_type & PHP_UV_UDP) {
        ZEND_PARSE_PARAMETERS_START(2, 3)
                ...
                UV_PARAM_OBJ(addr, php_uv_sockaddr_t,
                             (ip_type == PHP_UV_UDP_IPV4) ? uv_sockaddr_ipv4_ce : uv_sockaddr_ipv6_ce)
                Z_PARAM_OPTIONAL
                Z_PARAM_LONG(flags)
        ZEND_PARSE_PARAMETERS_END();
    } else {
        ZEND_PARSE_PARAMETERS_START(2, 2)
                ...
                UV_PARAM_OBJ(addr, php_uv_sockaddr_t,
                             (ip_type == PHP_UV_TCP_IPV4) ? uv_sockaddr_ipv4_ce : uv_sockaddr_ipv6_ce)
        ZEND_PARSE_PARAMETERS_END();
    }
...
static void
php_uv_handle_open(int (*open_cb)(uv_handle_t *, long), zend_class_entry *ce, INTERNAL_FUNCTION_PARAMETERS) {
    php_uv_t *uv;
    zval * zstream;
...
static void php_uv_udp_send(int type, INTERNAL_FUNCTION_PARAMETERS) {
    ...
    if (addr->std.ce == uv_sockaddr_ipv4_ce) {
        uv_udp_send(&w->req, &uv->uv.udp, &w->buf, 1, (const struct sockaddr *) &PHP_UV_SOCKADDR_IPV4(addr),
                    php_uv_udp_send_cb);
    } else {
        uv_udp_send(&w->req, &uv->uv.udp, &w->buf, 1, (const struct sockaddr *) &PHP_UV_SOCKADDR_IPV6(addr),
                    php_uv_udp_send_cb);
    }
}
...
static HashTable *php_uv_get_debug_info(zend_object *object, int *is_temp) {
    ...
    if (uv->std.ce == uv_poll_ce) {
        if (!*is_temp) {
            int fd;
            if (uv_fileno(&uv->uv.handle, (uv_os_fd_t *) &fd) ==
                0) { /* not actually a fd on windows but a handle pointr address, but okay. */
                *is_temp = 1;
...
PHP_FUNCTION (uv_signal_start) {
    ...
    if (uv_is_active((uv_handle_t *) &uv->uv.signal)) {
        php_error_docref(NULL, E_NOTICE,
                         "passed uv signal resource has been started. you don't have to call this method");
        RETURN_FALSE;
    }
...
PHP_FUNCTION (uv_signal_stop) {
    ...
    if (!uv_is_active((uv_handle_t *) &uv->uv.signal)) {
        php_error_docref(NULL, E_NOTICE,
                         "passed uv signal resource has been stopped. you don't have to call this method");
        RETURN_FALSE;
    }
...
PHP_FUNCTION (uv_accept) {
    ...
    if (server->std.ce != client->std.ce) {
        php_error_docref(NULL, E_WARNING, ".");
        zend_internal_type_error(ZEND_ARG_USES_STRICT_TYPES(),
                                 "%s expects server and client parameters to be either both of type UVTcp or both of type UVPipe",
...
PHP_FUNCTION (uv_timer_start) {
    ...
    if (uv_is_active(&uv->uv.handle)) {
        php_error_docref(NULL, E_NOTICE,
                         "Passed uv timer resource has been started. You don't have to call this method");
        RETURN_FALSE;
    }
...
PHP_FUNCTION (uv_timer_stop) {
    ...
    if (!uv_is_active(&uv->uv.handle)) {
        php_error_docref(NULL, E_NOTICE,
                         "Passed uv timer resource has been stopped. You don't have to call this method");
        RETURN_FALSE;
    }
...
PHP_FUNCTION (uv_udp_set_membership) {
    ...
    error = uv_udp_set_membership(&uv->uv.udp, (const char *) multicast_addr->val, (const char *) interface_addr->val,
                                  membership);
...
PHP_FUNCTION (uv_stdio_new) {
    ...
    if (handle == NULL || Z_TYPE_P(handle) == IS_NULL) {
        ...
        if ((flags & (UV_CREATE_PIPE | UV_INHERIT_STREAM)) == (UV_CREATE_PIPE | UV_INHERIT_STREAM) ||
            !(flags & (UV_CREATE_PIPE | UV_INHERIT_STREAM))) {
            php_error_docref(NULL, E_WARNING,
                             "flags must be exactly one of UV::INHERIT_STREAM or UV::CREATE_PIPE for UV handles");
            RETURN_FALSE;
        }
#if PHP_VERSION_ID >= 80000 && (!defined(PHP_WIN32) || (defined(HAVE_SOCKETS) && !defined(COMPILE_DL_SOCKETS)))
    } else if (socket_ce && Z_TYPE_P(handle) == IS_OBJECT && Z_OBJCE_P(handle) == socket_ce &&
               (socket = (php_socket *) ((char *) (Z_OBJ_P(handle)) - XtOffsetOf(php_socket, std)))) {
        fd = socket->bsd_socket;
        if (flags & (UV_CREATE_PIPE | UV_INHERIT_STREAM)) {
            php_error_docref(NULL, E_WARNING,
                             "flags must not be UV::CREATE_PIPE or UV::INHERIT_STREAM for socket objects");
            RETURN_FALSE;
        }
        flags |= UV_INHERIT_FD;
#endif
        ...
        php_error_docref(NULL, E_WARNING,
                         "passed unexpected value, expected instance of UV, file resource or socket object");
        RETURN_FALSE;
    }
...
PHP_FUNCTION (uv_queue_work) {
    ...
    php_error_docref(NULL, E_ERROR,
                     "this PHP doesn't support this uv_queue_work. please rebuild with --enable-maintainer-zts");
#endif
}
/* }}} */
