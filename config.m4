...
PHP_ARG_ENABLE(dtrace, Whether
to enable
the "dtrace" debug,
[ --enable-
dtrace Enable
"dtrace" support], no, no)
...
PHP_NEW_EXTENSION(fileio, fileio
.
c $COMMON
/fill_event_handle.
c $COMMON
...
/fs_handle_map.
c $FILES
/fill_fs_handle_with_data.
c $FILES
/file_get_contents_async.
c $FILES
/file_put_contents_async.
c $FUNCTIONS
...
if
test $PHP_UV
== "yes" && test -x "$PKG_CONFIG" && $PKG_CONFIG --
exists libuv1;
then
...
PHP_ADD_INCLUDE($UV_DIR

/include)
PHP_CHECK_LIBRARY(uv, uv_version,
[
PHP_ADD_LIBRARY_WITH_PATH(uv, $UV_DIR
/$PHP_LIBDIR, UV_SHARED_LIBADD)
AC_DEFINE(HAVE_UVLIB,
1,[ ])
],[
AC_MSG_ERROR([
wrong uv
